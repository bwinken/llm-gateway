...
- **Unified `/v1/*` surface** — One base URL exposes both backends. `/v1/chat/completions`, `/v1/messages`, `/v1/messages/count_tokens` dispatch by `model` alias: vLLM by default, Azure when the alias is configured under `[azure_models.*]` AND the caller has `can_use_azure`. `/v1/models` merges Azure aliases in for those callers so Claude Code's model picker shows both backends. Every route is also exposed without the `/v1` prefix (`/chat/completions`, `/messages`, ...) for clients whose base URL omits it. `/v1/chat/completions/render` (on-prem vLLM only) renders a request through the model's chat template and returns the resulting `token_ids` + resolved `sampling_params` **without generating** — a debug aid for seeing exactly what the model receives. Note `token_ids` is a list of token IDs, not decoded text, so reading the prompt as a string means detokenizing them yourself; a downstream vLLM without the render server answers 404
