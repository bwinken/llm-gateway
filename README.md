...
An accepted level is forwarded untouched. Anything else is sent as whatever `reasoning_effort_map` names for it, and a level the map doesn't name is **stripped** — the parameter is removed and the model uses its own default. There is no nearest-level guessing: the gateway sends what was asked for, what you wrote down, or nothing. `reasoning_efforts = []` strips every level; omitting the key keeps the previous behavior of forwarding whatever was requested. Works on all three backends and on both the OpenAI and Anthropic surfaces. A model that declares nothing is forwarded exactly as the client sent it, spelling included — the gateway does not decide what your downstream accepts. Within a declaration, `extra-high` / `x-high` are read as `xhigh`, so write the canonical name. `max` is **not** a spelling of `xhigh` — it is the level above it (Claude Code's top setting, and GPT-5.6's), so a deployment that stops at `xhigh` needs `reasoning_effort_map = { max = "xhigh" }` if `max` should land there instead of being stripped. The same keys are accepted on `[azure_models.*]` and `[bedrock_models.*]` entries. vLLM and Azure entries can be edited from **Admin Panel → Model Config → Advanced**, which shows each level as a chip and spells out what happens to a rejected one.
