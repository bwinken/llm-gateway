...
- **System consolidation (request direction)**: all system text — the top-level `system` field **and** any `role: "system"` / `role: "developer"` entries that appear inside `messages` (newer Claude Code injects mid-conversation system reminders this way) — is collected and emitted as a **single** system message at index 0. A system message left at a non-zero index makes strict downstream chat templates (e.g. Qwen3.x) return `400 "System message must be at the beginning"`, so the gateway never emits one anywhere but the front.
