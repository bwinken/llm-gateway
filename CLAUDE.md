...
- **Streaming is NOT SSE** — ConverseStream returns binary `application/vnd.amazon.eventstream` frames. `app/services/aws_eventstream.py` is a self-contained ~150-line decoder (prelude/headers/payload + CRC32 checks; `encode_event` exists for tests) so botocore is not a dependency. `_pump_bedrock_events` mirrors `_pump_anthropic_lines` (same 10s ping heartbeat / 300s max-idle) but feeds bytes through `EventStreamDecoder` and yields `('event', (event_type, payload))` tuples; mid-stream exceptions arrive as `:message-type=exception` frames and are surfaced via `ConverseToChatStreamTranslator.derive_error_message()/derive_error_kind()` (throttling/service-unavailable → `overloaded_error`, validation → `invalid_request_error`).
...
- vLLM downstream HTTP calls use a single shared `httpx.AsyncClient` (initialized in lifespan, accessed via `server_state.get_client()`); Azure downstream calls use `server_state.get_azure_client()` and Bedrock calls `server_state.get_bedrock_client()`, each returning a separate proxied/insecure client when its `*_HTTP_PROXY` / `*_INSECURE` env is set, falling back to the shared client otherwise
