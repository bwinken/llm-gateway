...
- Tables: `users` (with auto-generated `api_key`, `display_name`, `org_code`, boolean access-control flags `is_disabled` and `can_use_azure`, plus nullable `azure_daily_limit_usd` — the Azure daily sub-limit, see Cost Calculation), `usage_logs` (with covering index on `user_id + created_at + cost_usd`, plus `backend` "vllm"/"azure" tagging which downstream served each request), `app_owners` (many-to-many: which users own which app accounts)
