...
| `TestCanonicalSpelling` | `canonical_effort` folds the xhigh spellings (`extra-high` / `x-high` / `extra_high`, any casing), keeps `max` as `max`, and returns an unknown level or a non-string verbatim — spelling is canonicalized, levels never are |
...
| `TestApplyHelpers` | In-place rewrite/strip of OpenAI `reasoning_effort` and Azure Responses `reasoning.effort` (emptied `reasoning` object removed); bodies without the field, and undeclared routes, untouched apart from the unconditional spelling fold (`extra-high` → `xhigh`, `max` and an unknown level left alone); a declared route accepts a variant spelling of a level it lists |
| `TestVllmChatCompletions` | `/v1/chat/completions` strips an unaccepted effort, sends the mapped one, forwards an accepted one unchanged, leaves an undeclared route's request alone, forwards a variant spelling canonically, and forwards `max` as `max` |
