...
| `TestCanonicalSpelling` | `canonical_effort` folds Claude Code's xhigh spellings (`max` / `extra-high` / `x-high`, any casing) and returns an unknown level or a non-string verbatim — spelling is canonicalized, levels never are |
...
| `TestApplyHelpers` | In-place rewrite/strip of OpenAI `reasoning_effort` and Azure Responses `reasoning.effort` (emptied `reasoning` object removed); bodies without the field, and undeclared routes, untouched apart from the unconditional spelling fold (`max` → `xhigh`, an unknown level left alone); a declared route accepts a variant spelling of a level it lists |
| `TestVllmChatCompletions` | `/v1/chat/completions` strips an unaccepted effort, sends the mapped one, forwards an accepted one unchanged, leaves an undeclared route's request alone, and forwards a variant spelling canonically |
...
| `TestBedrockAdaptation` | The mapped level is resolved before Converse expands it into a Claude thinking budget (`xhigh` → mapped `medium` → 8192); an unmapped level leaves `additionalModelRequestFields` off entirely; a variant spelling (`max`) buys the full xhigh budget instead of falling through to the medium default |
